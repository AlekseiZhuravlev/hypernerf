...
def main(argv):
    ...
    if eval_config.num_train_eval == -1:
        ...
        print('len(datasource.train_ids)', len(datasource.train_ids))

        # train_eval_ids = np.array(sorted(
        #     np.random.choice(datasource.train_ids, eval_config.num_train_eval, replace=False),
        #     reverse=False))
        train_eval_ids = datasource.train_ids[:eval_config.num_train_eval*2:2]
